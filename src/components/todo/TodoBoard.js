...
function LogMessage(props: { msg: string, color?: string }) {
  ...
  return (
    <div className="todo-board-log" style={colorStyle}>
      {msg}
    </div>
  );
}
...
> {
  ...
  logNewMsg = (msg: string) => {
    if (msg) {
      ...
      const newLogMsg =
        msg instanceof Error ? `> ${msg.name}: ${msg.message}` : `> ${msg}`;
  ...
  handleBoardClick = (e: SyntheticMouseEvent<HTMLDivElement>) => {
    ...
    if (
      !elHasClass(clickedEl, 'todo-create-new') &&
      !elChildOfClass(clickedEl, 'todo-create-new')
    ) {
    ...
    return (
      <div className="todo-board">
        <section className="todo-board-top-control">
          <div id="todo-board-logs">{logMsgEls}</div>
          <div className="todo-board-title">To-do tracker</div>
          <TodoCreateNew
            key={todoCreateNewKey.toString()}
